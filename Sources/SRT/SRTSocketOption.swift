...
public enum SRTSocketOption: String {
    static func from(uri: URL?) -> [SRTSocketOption: Any] {
		...
		options[.streamid] = options[.streamid] ?? uri.absoluteString.split(separator:"/").last?.split(separator:"?").first
        return options
    }
    ...
    func apply(_ socket: SRTSOCKET, value: Any) -> Bool {
		...
		let result = data.withUnsafeBytes { (ptr: UnsafeRawBufferPointer) -> Int32 in
			...
			return srt_setsockopt(socket, 0, symbol, bytes.baseAddress, Int32(data.count))
        }
        return result != -1
    }
